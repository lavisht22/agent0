...
**Status:** Phase 1c **code-complete** — the web app no longer makes any direct Supabase *data* access (`.from`/`.rpc`/`.storage` all gone; only auth `getSession`/`getClaims`/`signOut`/OTP remain, by design). All tables migrated onto the runner API. Two runner additions were needed for the agents cluster (`staging_model`/`production_model` on `/agents`; `DELETE /agents/:id`). **Outstanding before Phase 1 is truly done: a manual regression pass** (changes are tsc+biome clean but unverified at runtime). Then Phase 2 — replace Supabase Auth with better-auth.
...
**Status:** 1a + 1b + 1c code-complete (tsc+biome clean); **manual regression pass still outstanding** before declaring Phase 1 done.
...
- [ ] Regression pass: every screen works through the API — **not yet done (needs manual verification; see risks below).**
