...
**Status:** In progress — Phase 1b **complete** (all runner API gaps filled: providers, mcps, api_keys, PATs, workspaces+members, dashboard RPCs; runs/tags coverage confirmed). Next: Phase 1c — migrate the web app off direct Supabase data access onto these APIs.
...
**Status:** In progress — 1a + 1b shipped; 1c not started
...
- [ ] `queries.ts`: ~~workspaces~~ ✅ · ~~workspace_user~~ ✅ · agents ⏳ · agent_versions ⏳ · agent_tags ⏳ · ~~tags~~ ✅ · ~~providers~~ ✅ · ~~mcps~~ ✅ · ~~runs (+log download)~~ ✅ · ~~api_keys~~ ✅ · ~~personal_access_tokens~~ ✅ · ~~users~~ ✅ (folded into members) · ~~dashboard RPCs~~ ✅
- [ ] Per-route direct calls — done for all migrated tables; **remaining: the agents cluster** (`agents.index`, `agents.$agentId` + its `use-agent-mutations`/`tools-section`, `agent-filter`, `version-history` already moved off member lookups). ⚠️ **Blocked on a runner API gap — see progress log 2026-06-04 agents entry.**
